...
namespace Alachisoft.NCache.Web.SessionState
{
    public static class NCacheSessionExtensions
    {
        ...
        public static void Set(this ISession session, string key, object value)
        {
            ...
            if (session is NCacheSession)
            {
                if (((NCacheSession)session).UseJSON)
                    binary = false;
            }
            if (binary)
            {
            ...
            {
                var serializableObject = JsonConvert.SerializeObject(value, JsonSessionSerializationSettings.Instance);
                var data = Encoding.UTF8.GetBytes(serializableObject);
                session.Set(key, data);
            }
        ...
        public static bool TryGetValue(this ISession session, string key, out object value)
        {
            ...
            if (session.TryGetValue(key, out data))
            {
                try
                {
                    ...
                    {
                        value = JsonConvert.DeserializeObject<object>(Encoding.UTF8.GetString(data), JsonSessionSerializationSettings.Instance);
                    }
