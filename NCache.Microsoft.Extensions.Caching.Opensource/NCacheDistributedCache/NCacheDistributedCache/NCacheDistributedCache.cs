...
{
    public class NCacheDistributedCache : IDistributedCache, IDisposable
    {
            ...
            try
            {
                ...
                if (options != null)
                {
                    if (options.AbsoluteExpiration != null)
                    {
                        ...
                        DateTime targetTime = options.AbsoluteExpiration.Value.DateTime;

                        // difference from now to target time
                        TimeSpan difference = targetTime - DateTime.UtcNow;

                        // if already expired, avoid negative timespan
                        ...
                        cacheItem.Expiration = new Runtime.Caching.Expiration(Runtime.Caching.ExpirationType.Absolute, difference);
