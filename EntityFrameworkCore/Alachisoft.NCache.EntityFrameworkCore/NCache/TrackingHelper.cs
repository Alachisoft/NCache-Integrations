...
namespace Alachisoft.NCache.EntityFrameworkCore.NCache
{
    class TrackingHelper
    {
        ...
        internal void SafeAttach(EntityReferenceMap entityReferenceMap, object entityToBeAttached)
        {
        ...
        private void SafeAttachInternal(EntityReferenceMap entityReferenceMap, object entityAtHand, HashSet<object> entityTrack)
        {
            ...
            foreach (MemberInfo member in GetRelationMembers(entityAtHand))
            {
                ...
                if (relationMemberValue != null)
                {
                    ...
                    Dictionary<object, InternalEntityEntry> unchangedReferenceMap = (Dictionary<object, InternalEntityEntry>)entityReferenceMap.GetType().GetField("_unchangedReferenceMap", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(entityReferenceMap);
                    if(unchangedReferenceMap != null)
                    {
                        unchangedReferenceMap.Keys.ToList().ForEach(action);
                    }
        ...
        private void TypeTracking(Dictionary<object, InternalEntityEntry> entityReferenceMap)
        {
            IList<string> typeNames = new List<string>();

            Action<object> action = new Action<object>(delegate (object entity)
            {
                Type entityType = entity.GetType();

                if (!typeNames.Contains(entityType.FullName))
                {
        ...
        private void SetThingsUpForAttachment<T>(EntityReferenceMap entityReferenceMap, T entityStateManager, HashSet<T> hsEntitiesFromTracked, HashSet<object> entityTrack)
        {
