...
namespace Alachisoft.NCache.EntityFrameworkCore
{
    public static partial class DbContextExtensions
    {
        ...
        {            
            EntityReferenceMap _entityReferenceMap = (EntityReferenceMap)stateManager.GetType().GetField("_entityReferenceMap", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(stateManager);
            Dictionary<object, InternalEntityEntry> _unchangedReferenceMap = (Dictionary<object, InternalEntityEntry>)_entityReferenceMap.GetType().GetField("_unchangedReferenceMap", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(_entityReferenceMap);
            ...
            while ((entity = trackingHelper.DequeueEntity()) != null)
            {
                Func<object, bool> func = delegate (object entityStateManager)
                {
                    ...
                    string methodName = nameof(trackingHelper.EntityComparer);//EntityComparer
                ...
                if (_unchangedReferenceMap == null)
                    ...
                    mapVal = _unchangedReferenceMap.Keys.Where(func).FirstOrDefault();
